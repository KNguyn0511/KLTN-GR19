...
//       // 3. LƯU TOKEN ĐỂ TRÌNH DUYỆT NHỚ
//       localStorage.setItem("access_token", response.access_token);

//       // 4. LƯU THÔNG TIN USER VÀO ZUSTAND
//       // Backend của bạn đang trả về { access_token, user: { ... } }
//       // Bạn ném cục user này vào hàm login của store để Header cập nhật tên/ảnh đại diện
//       login(response.user);
...
//       // Bắt lỗi từ Backend trả về (VD: "Sai mật khẩu")
//       toast.error(
...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormData) => {
    try {
      // 1. GỌI API THẬT (Dùng cấu trúc mới của connect-db)
      const res = await authApi.login(data);

      // 2. LƯU TOKEN ĐỂ TRÌNH DUYỆT NHỚ (Giữ lại code của bạn để đảm bảo an toàn)
      localStorage.setItem("access_token", res.access_token);

      // 3. Giải mã JWT để lấy _id (Vì backend không trả _id trực tiếp trong user object)
      const decoded = decodeJwtPayload<{ id: string; email: string; role: string }>(
        res.access_token,
      );

      // 4. LƯU THÔNG TIN VÀO ZUSTAND (Theo đúng format mới mà store đang yêu cầu)
      login({
        _id: decoded?.id ?? null,
        email: res.user?.email ?? data.emailOrPhone,
        fullName: res.user?.fullName ?? "",
      ...
      // Lời chào cá nhân hóa từ nhánh connect-db
      toast.success(`Chào mừng ${res.user?.fullName ?? "bạn"} trở lại!`, { autoClose: 2000 });
      router.replace("/");
    } catch (error: any) {
      console.error("Lỗi đăng nhập:", error);
      // Gộp câu báo lỗi chi tiết của nhánh kia và thời gian hiển thị của bạn
      const msg =
